...
/// # ⭐ 闩是**每台各自**的,⛔ 不随快照传播
///
/// 「本账户全员已具备」是条账户级**事实**,但这一行是**本机自己观测到它的记录**:
/// `boot::import_attached` 只往本地 `sync_meta` 写 `bootstrapped_at` 与 `epoch` 两键
/// (`boot.rs`),**从不整表复制对端那份** ⇒ 新加入的设备一律**从没闩过**开始,得自己
/// 观测一次。那是 fail-closed 的方向,也正是我们要的:它对「谁在这个账户里」的了解
/// 还是空的。⛔ 别为了「同一个事实何必各观测一次」把它挪进同步面 —— 那就成了一条
/// 可被对端写的授权。
///
