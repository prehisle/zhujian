...
- **洪泛闸(2026-07-31 防御评审补,codex 一轮修法)**:① **未鉴权截止**——Fresh 态连接须在 **15s** 内送达完成鉴权的那一帧,否则断开(**绝对钟,任何帧都不续命**——静默判死把 WS ping 也算活动,防不了「只 ping 保活的空连接」永久占位;截止前一瞬到达的帧仍会被处理完,超出量=单帧处理时长;PairJoined 放宽到槽 TTL+30s,authed 解除只剩静默判死);**槽死即踢 joiner**(owner 关槽/断线/sweep/烧槽的公共汇点统一 kick)——未鉴权 joiner 只为槽活着,不许沦为可循环铸造的僵尸连接;② **全局并发连接硬上界 32**(组合包络由启动校验用真实尺寸闭合:hub 统一预算 + max_conns×[下行 channel 槽位×size_of(ServerMsg) + 2×1MiB 帧/解码副本] ≤ 448MiB=MemoryMax−基线预留,超即拒启;upgrade 前取 permit、满=503,停机关栅即 close 信号量与拒新同一线性化点;permit 由连接读任务 RAII 持有到死;生产单元须配 `LimitNOFILE > max_conns+预留`,deploy §1);③ **创号闸**——账户目录硬上界 **10000**(到顶=运营容量事件;启动带超额目录即 ERROR 告警、存量用户照常服务)+ 令牌桶 **20 深、每分钟补 1 枚**(**只对「真要新建账户」花令牌**:幂等重试/not_first/老用户的 auth 与背书注册全不经闸),两错对外同一个 `busy` 后断开,拒绝日志 60s 聚合防放大。**已知余量**:全局桶可被持续洪泛垄断→新创号一直 busy(存量用户不受影响),对策=运营观察+封禁,不为此加每源识别(服务端在反代后看不到真实源 IP)。背景:开放创号下每次创号全量重写 registry.json 且账户条目永不回收,洪泛防线不能只靠反代/系统层。
...
- **块大小是合规判据,不只是发送方的习惯**(264 起收端严格校验;lan-direct-plan §10 的 C′ 收口)。`bytes` 一旦声明,「共几块、每块多长、哪一块带 `last`」就唯一确定:`total = ⌈bytes / 262144⌉`,`idx ∈ [0,total)` 且必须等于收端的下一个待收序号,**非末块 `len(data)` 恒 262144、末块恒 `bytes − idx×262144`,`last` 当且仅当 `idx+1 == total`**。任一不符 = 与错序同一收口(作废本次拉流、shun 这条 (来源,腿)、图回队列并当场重问),**不是**静默丢。
  - 为什么升成硬判据:原先只查「序号连号 ∧ 不超声明字节」,于是一串 `data: []`、`last: false` 的空块**全部合法通过**,而每一枚又把「无进展」计时清零——攒块永不增长、传输永不结束、也永不判死。校验之后每一枚被收下的块都必然推进整整一块,故「无进展」超时同时就成了**最低有效进度**规则(≥256 KiB / 每 idle 窗口),整笔时长因此有界且与图大小成正比。
  - 现网各版本的发送侧本来就按 262144 切块,故这条**不构成版本偏斜**;但**另写实现必须照此切块**,否则会被拒收并反复重试。
- **供流侧不得整图物化**(264;lan-direct-plan §10):按 `(rowid, image_id, offset)` 增量取块、发一块丢一块,发送窗口 = 1 块;中途行消失即沿同 transfer 回 `blob_deny`。**收端同时至多一笔在飞拉流**——这条是本机策略(封住「缺 N 张图就攒 N 份最大 32 MiB」的收端内存面),**不是协议约束**,别的实现自行取舍。
...
- **引导完成后必须装配 `Engine` 并走一次中转会话仪式**(256 起 = `EngineSlot::reconcile` + `on_relay_session_up`):引擎 pending 池出队条件是严格 `seq == watermark+1`,导入抬高水位后池内旧队头永不出队会堵死该 origin;引擎状态本就可丢,重建 + 重发 hello 即步骤 6「pending 自然续上」的兑现形,水位不过缺口零丢失。
...
- **引导编排**:引导中(bootstrapped_at 缺席)op/ctl/blob 帧**整帧丢弃**(半路应用会把库变「非 fresh」永久堵死导入;引导完成后 hello 互补重取,零丢失);Req 发给首个在线同伴,30s 无 Offer/块间超时轮转下一台(自己也在引导时不应答 Req,并发引导靠超时轮转无死锁);**一台同伴都不在线**时保持 booting 等 Peer 事件,最后一枚中转帧之后静默满一个步长就把「没有在线设备可提供初始快照」写进状态面 `boot_hint`(⛔ 刻意不占 `error`,两格可同屏)、同伴一上线即收回、会话结束即清(用户面 121;句子唯一出处 `NO_BOOT_PEER_HEAD`,「加入空间」仪式与手机「全部同步」同用);import 一次持锁(fresh→commit),成功后装配 Engine + 走会话仪式(§6.2/P2-f 契约的兑现处)。
...
- **⚠ 局域网直连(L 系列)叠加了一层本文没覆盖的观察者**:上面整节默认「网络上唯一的观察者是服务器」,而直连之后**同一局域网里的旁观者**也能看见 `Frame{from,to}` 的明文设备 ULID、设备↔内网 IP 的本地拓扑、流量时刻与大小——**这一面强于服务器可见面**。内容机密性与完整性不弱于经恶意中转(丢帧=水位互补 / 重放=op_id 幂等 / 改序=per-origin 校验 / 拉流劫持=transfer 绑定+路由降级),**新增的是可用性与元数据两面,与中转不可比**。完整增补见 [lan-direct-plan §9](lan-direct-plan.md#9-威胁模型增补叠在-sync-protocol-11-上),对外披露文案见该文 §9.1。
