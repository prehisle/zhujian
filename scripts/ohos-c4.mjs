...
//   ⇒ 第 n 枚中心 y = 119 + (60 + (n-1)×48 + 24) × 3.23  ⇒ FIRST_Y 曾是 390
// ⛔⛔ **657 把这套坐标的原点挪了**:`EntryAbility.ets` 改成 `setWindowLayoutFullScreen(true)`,
//    窗口从安全区内变成整屏 ⇒ 上面那个 119 的内缩**没有了**,面板从屏幕 y=0 起排。
//    推算新值 = (60 + 24) × 3.23 ≈ **271**,⚠ **推的、没重量过** —— 本文件头一句就是
//    「这三个数是量出来的,不是算出来的」,所以这里刻意**归零**让它走下面那条 fail-closed 路:
//    下一个要用 C4 的人先 `shot calib` 量一次、填回来。⛔ 别把 271 直接填进去当量过的。
// ⭐ **这两个数有活的自检**:点第 n 枚之后,日志里会说它跑的是哪条命令 ——
//    点 3 却看见 `C4 schema` 就是坐标歪了,⛔ 别靠肉眼比对截图。
const TAP_X = 630; // 屏宽 1260 的中线;按钮通栏,x 取中间
const FIRST_Y = 0; // 第 1 枚按钮中心 y —— ⛔ 657 起待重量(见上),0 = 「还没量」
const STRIDE = 155; // 相邻两枚中心 y 之差(48 CSS px × 3.23);原点挪了它不变
