...
// # 为什么是三份而不是一份
//
// 官网那份**物理上合不成一份**:静态单文件、scp 部署,引用不了仓内别处的 CSS;安卓那份立闸时
// 判的是「独立 vite 工程,要引仓根文件得动 fs.allow + rollup 输入 + tauri 打包三处」—— ⚠ 673 实测
// 只有 `server.fs.allow` 一处(673 起 TS 纯逻辑可以搬进仓根 `shared/`、两端 import 同一份(只要安卓 dev server 放行仓根;rollup 与 tauri 打包一行不用改),搬不搬见 backlog 代码与结构 8),但 CSS 令牌表是内联进 `android/index.html` 的,
// 今天仍是三份。最多做到「一份源 + 构建期复制到三处」,而复制出来的副本**照样会在有人手改时漂**
...
桌面那边分段控件走的是另一套(.seg 只在手机端)",
  ...
  },
  {
    ...
    in: APPS,
    dark: true,
    why: "朱砂当小字、直接压在纸面(--paper / --raised / --raised-edge 渐变)上时的字色(用户面 132):--seal 压这几层底够不上 §2.2 小字 5.0(亮压纸底 4.93、渐变底端 4.75;暗压 --raised 4.87、底端 4.41),往 --ink 压到最不利那层底 ≥ 5.0(亮 4.5% / 暗 13.1%)。⛔ --seal 本身不动(按钮 / 描边 / 淡朱底在用)。两个客户端输入令牌同值 ⇒ 同名同值;官网没有这一族小字",
  },
  {
    name: "--seal-ink-on-seal-tint",
    in: APPS,
    dark: true,
    why: "朱砂字压在自己那层淡朱底(--seal-tint / 染色卡里的 --seal-tint-solid)上的 chip / 选中态字色(用户面 133):亮档往 --ink 压 16.3% 使淡朱底叠在最不利那层纸面(渐变底端 --raised-edge)上 ≥ §2.2 小字 5.0;暗档刻意等于暗档 --seal(用户看对比图拍板:压到过线会变橙粉、与实心逾期 chip 不再同一个红)。两个客户端输入令牌同值 ⇒ 同名同值;官网没有这一族",
  },
  ...
  // 也⛔ 别在官网复用 --paper-veil 这个名字 —— 同名不同值会让这道闸从此看不见它们(登记表按名字比)。
  // ⚠ 644 补:这段原本写着「安卓那层没有模糊」——**是错的,当轮没查就写下的**
  // (`android/index.html` 的 header 与 #bottombar 都有 `backdrop-filter: blur(6px)`,
  // 官网是 blur(8px))。⇒ 两份的差只有 alpha 一项,别再拿「有没有模糊」当理由。
  {
    name: "--topbar-veil",
    in: ["官网"],
    dark: true,
    why: "官网吸顶栏那层「纸纱」= --paper 带 82% alpha(安卓同族那枚是 94%,⛔ 别互相对齐)。\
⛔ 别改回 `color-mix(… var(--paper) …)`:\
含 var() 的值到**计算期**才失败(IACVT)⇒ 属性归 unset、⛔ 不回退到前一条兜底。而官网是**任意手机\
浏览器**打开的(Chrome ≤110 / Safari ≤16.1 / 微信 X5 那一族都不认 color-mix)—— 84 量出来:退成\
unset 之后顶栏是**字叠字**,同屏最大色差 208/255(2x 设备像素)。桌面/安卓的顶栏各有自己的一份",
  },
  {
    name: "--band-veil",
    in: ["官网"],
    dark: true,
    why: "官网「几件事先说清」那条横带的底 = --raised 带 55% alpha;来路同 --topbar-veil。\
⚠ 它退成 unset 只是掉一层极淡的抬色(84 量:最大色差 16/255,上下两条 --line 分隔还在)——\
**跟着一起修是因为同一份文件里不该两种写法**,不是因为它难看。app 两端没有这条横带",
  },
  {
    name: "--seal-select",
    in: ["官网"],
    dark: true,
    why: "官网选区底 = --seal 带 26% alpha(比 --seal-tint 的 10% 深一档,理由见桌面 src/theme.css\
那条 ::selection 注释)。⚠ 退成 unset 时选区底是 **transparent 而不是 UA 默认蓝** ⇒ 选中了看不出来,\
这一处是**功能**不是装饰。桌面那份 ::selection 今天仍写着 color-mix —— 它跑在 WebView2(常青)上,\
不在 84 的面内,⛔ 别顺手改成引这个官网令牌",
  },
  {
    name: "--shot-veil",
    in: ["官网"],
    dark: false,
    why: "官网「点开看大图」那层浮层的遮罩底(653)。⛔ **刻意不随明暗档翻面**,所以 dark:false ——\
它压的是截图不是纸面,亮色档下翻成浅色 = 图与底糊在一起、也不再有「这是一层浮层」的信号。\
值与桌面客户端那只看大图窗(src/lightbox.css 的 .img-lightbox)逐字相同;桌面那处今天是**写死**的、\
不是令牌,⛔ 别为了对齐把它改成引这一枚 —— 两个工程各自独立部署,官网引不到 src/ 的 CSS",
  },
  {
    name: "--sat",
    in: ["安卓"],
...
FAB 恰 16px,跑手 scripts/cdp-acceptance-wide-column.mjs)。⛔ 只在安卓那份 —— 桌面走的是\
容器查询(504)与侧栏折叠(501),不是同一套机制;鸿蒙壳共用安卓这棵前端树,故自动吃到",
  },
];
