...
(async () => {
  ...
  // ⛔⛔ **别退回成 `!#topics-new.hidden`**(627 实测逮到、629 改掉的那个错判据):
  // app 刚起、标签面**一次都没开过**时,`#topics-new` 的 `hidden` 本来就是 false(它只在合并态里
  // 才被置 true)⇒ 那句会把关着的面报成「开着」⇒ 下面「开着就先收起来」反而把面**打开**,
  // 随后 `toggle()` 又把它关上,整支从第一格起全跑偏。627 之前它没红,只因为跑之前多半有人
  // 手动开过那个面 —— 一格**靠运气绿着**的判据。
  // ⇒ 判据只认**看得见没有**;`cdp-acceptance-topic-kids.js` 用的就是这条。
  // ⛔ 也别照抄别的面那五支的 `body.pane-open`:标签面不走那个类。
