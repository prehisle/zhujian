...
- **Task board**: four columns by default — **To do / In progress / To confirm / Done** — and **the columns are yours to change** —
  ...
  - **Custom columns** (desktop): “Columns” in the board header lets you **add, rename, drag into order and delete** columns, and they sync to your other devices (phones show them and move cards between them as usual — adding and removing is done on the desktop, since reshaping a board is a rare, fiddly thing to do on a touchscreen). Two rules: **To do and Done can be renamed but not deleted** (new tasks land in To do, and only Done stamps a task finished and lets it go to the Archive); and **a column must be emptied before it can be deleted**. ⚠ With more than one device this stays closed until **every device on the account has upgraded to a version that understands it** (an older device would not know what to make of a new column) — while it is closed the panel says so.
  - **To confirm** (the fourth column by default): work that is finished but waiting on someone else, separated out from In progress. It is **never mandatory** — In progress → Done skips it, and columns move freely in both directions (things can be sent back). If you have no use for it, rename it or delete it.
  ...
  - **Never miss a deadline**: the board header shows an "M overdue · N due today" summary button (click it to collapse the board to just those cards); on desktop the app can also **post one system notification a day** with those numbers (Settings → General → Due reminder; on by default at 9:00, it only speaks when something is actually due or overdue, and the time is adjustable or the whole thing can be switched off. The switch and time belong to this device only; nothing fires while the app is closed).
  ...
  - **Archive**: finished work is history worth keeping, and should not share a bin with the trash — the Done column header archives **everything** in one go (two-step confirmation), or drag to the strip at the bottom, or use **Archive** in a card's menu. The archive view groups into a timeline **by the day the work was finished** (not the day it was filed, so archiving a week's work at once does not squash it into today), with a line of **stats** in the header (`Finished this week: N · N in total`). It can be read but **not deleted from** (to delete, **un-archive** back to the board first and then delete normally — one extra step, on purpose). The archive view is **read-only and quiet**: cards carry no standing buttons, and the one **Unarchive** action lives in the card menu like everywhere else (desktop: hover ⋯ / right-click / the `A` key; phone: tap the card). **The trash works the same way** — **Restore** and **Delete forever** live in the card menu too, rather than sitting on every card all year round.
