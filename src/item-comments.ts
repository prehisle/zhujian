// 条目留言(identity-plan §4)——灵感卡 / 任务看板卡共用的徽章 + 留言层,照
// item-images.ts 的形做成视图无关的共享件。
//
// ⭐ **601 起「同一个概念在两处长得一样、行为一样」这条不再成立,是有判据的分野**(原话
// 就写在这儿,别照旧读):随记(全宽单列)**就地长在卡片里**,看板(200px 起的窄列、列
// 自己还要滚)**仍是居中遮罩浮层**。判据 = **容器形状**,不是偏好;两形的差别与各自为什么
// 见 PanelMode 与 openInlineComments 的头注。⛔ 别为「一致性」把随记那侧改回浮层 —— 那是
// 拿正确性换一致性(identity-plan §4.7 与 progress-log 601)。
//
// 三条本模块特有的纪律:
...
//  2. **迟到响应判弃**:这一层每次打开领一个会话号,所有异步回调先核会话号再动 DOM。
...
//     到达,动不动 UI 由会话号判。
//
// 浮层那一形 portal 到 body:已登记进 hotkey-menu.ts 的 SATELLITE_LAYERS(卫星浮层白名单),
// 新增 portal 层也须进那份名单,否则点它会误触发别卡编辑态的默认保存。⚠ `.cm-panel` 也在
// 那份名单里,但它**不是** portal 层(inline 那形长在卡片内)—— 登记它是为了另一件事:
// 那张卡上若同时开着标签 / 截止选择器,点留言框不该把选择器收掉。
