...
// # 为什么是两份而不是一份
//
// 同 theme.css 那三份令牌表(理由详见 check-theme-drift 文件头):立这道闸时判的是「安卓是
// **独立 vite 工程**,合不成一份」⇒ 各写一份 + 门禁逐值比对。⚠ 那句前提 673 起 TS 纯逻辑可以搬进仓根 `shared/`、两端 import 同一份(只要安卓 dev server 放行仓根;rollup 与 tauri 打包一行不用改),搬不搬见 backlog 代码与结构 8;
// 这份今天还是两份,**新增/改动任一个值 = 另一份必须同步改**,否则门禁当场拒。
