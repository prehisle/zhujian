...
//
// **与桌面刻意不同的三处**(不是漂移,是这一端本来就没有那几样东西):
...
// - **没有缩进那一记**(600):软键盘上没有 Tab。⚠ 纯逻辑那份仍**两端逐字对应**
//   (`indentChecklistLines` 在这一端没人调,由 `check-filter-parity` 照样两端同压)——
//   ⛔ 别当成死代码删掉,那道闸对的是「两份纯逻辑一不一样」。要给这一端补手势得先有个
//   够得着的入口(⛔ 别为它凭空加一枚钮,先问用户)。
//
