...
// ⚠ 同一把尺曾经还用在 `e2e/probes/pri-fold.e2e.js` 与 **`e2e/specs/task-time.e2e.js`(真断言)**上
//   —— 那几枚钮的字短、不触发分段,恰好都是 1 个 rect ⇒ 那两处一直是绿的,但那是运气不是判据。
//   ⭐ **523 已把两处都换成「数不同的 y」**(并在同一棵树上复跑了 510 那把阴性对照刀),
//   ⇒ 这条尺今天全仓只剩一个诚实形。⛔ 别再往回改成数 rect 个数。
