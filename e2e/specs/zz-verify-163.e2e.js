...
// zz 前缀刻意钉在字典序末尾:例② 要在两个窗之间来回切、且量的是遮罩窗铺满显示器那一下的
// 视口时序,窗口几何敏感,放最后跑。⭐ **606 起遮罩住在另一只窗里**(lightbox.html):逐帧
// 记录器要装在**那只窗**上(它常驻,开图前就能装),点缩略图仍在笔记本窗做。
...
describe("163 · 手势即回执 + 布局未定不显示", () => {
  ...
  it("超高图点开:可见帧宽度恒为终态整图宽(无「先宽后缩」裸渲染帧)", async () => {
    ...
    // 先切到遮罩窗装逐帧记录器,再切回来点缩略图:每 rAF 记录查看器 img 的可见性与实际宽度,**并连视口一起记**。
    ...
    // 的现场(铺满显示器那一下落到视口上晚于 awaitViewport 放行 ⇒ 亮相后又排了一次)。
    const back0 = await toLightbox();
