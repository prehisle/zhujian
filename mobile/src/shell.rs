...
/// ⛔⛔ **`AppHandle::exit(0)` 在鸿蒙上是个空操作 —— 651 真机实测**:点「不同意」之后
/// 进程号一动不动,而屏幕上什么都不会发生(报错还被弹窗盖住了,见 index.html 里
/// `.error` 那条 z-index 注释)。⇒ 改成直接结束进程。
/// ⚠ **这里可以硬退,是因为时刻特殊**:这道告知挡在 `resolveStartupGate()` **之前**,
/// 用户还碰不到任何界面 ⇒ 没有一笔**用户发起的**写入在飞。⛔ 但别读成「后端也还没动」——
/// `setup_shell` 一起手就把装配 worker 派出去了,不等前端;老用户升级上来第一次启动时,
/// 前滚迁移 / 引导清扫 / 装配就在这道告知后面跑着(669 读代码核的;651 写的那句
/// 「空间还没装配」只在全新安装上成立)。硬退仍然安全,靠的是那几步各自的形:迁移整笔在
/// 一个事务里(`core/src/db.rs` 头注)、清扫幂等、租约本就按「手机常被系统 kill」设计 ——
/// 与系统在这一刻杀掉进程没有区别。
/// ⛔ 别把这条命令挪到别处用:用户一旦能操作,硬退就可能掐断一笔在飞的写入 = 丢数据。
/// ⚠ 两只手机壳共用本 crate ⇒ 安卓与鸿蒙同得同失;**真机上必须各点一次**
/// (「退出」这种事失灵的样子就是「什么也没发生」)。鸿蒙 651 真机点过;安卓 669 在
/// MuMu 上由 `scripts/cdp-acceptance-privacy-gate.mjs` 真触摸点过(含阴性对照)。
#[tauri::command]
pub fn app_exit() {
    std::process::exit(0);
}
