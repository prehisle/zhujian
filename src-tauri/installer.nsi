...
; zhujian: 快捷方式 / Add-Remove 的面向用户显示名走中文,与稳定的安装身份(PRODUCTNAME=
...
Function un.ConfirmShow ; zhujian: "Delete app data" checkbox intentionally NOT created.
  ; Desktop is the sole snapshot/recovery source (server is not a backup);
  ; the uninstaller must never be able to wipe the user's SQLite databases.
FunctionEnd
!define MUI_PAGE_CUSTOMFUNCTION_LEAVE un.ConfirmLeave
Function un.ConfirmLeave
  StrCpy $DeleteAppDataCheckboxState 0 ; zhujian: force off — never delete app data
FunctionEnd
...
Section Uninstall
  ...
  ${If} $UpdateMode <> 1
    ...
    ; zhujian: 中文显示名(SHORTCUTNAME)+ 残留英文名(PRODUCTNAME)两者都清。
    ...
    ; Remove desktop shortcuts (中文名 + 残留英文名)
  ...
  ${AndIf} $UpdateMode <> 1
    ...
    ; zhujian: user-data deletion removed on purpose — RmDir of $APPDATA/$LOCALAPPDATA\${BUNDLEID} disabled.
    ; The SQLite databases in %APPDATA%\${BUNDLEID} are the only recovery source and must survive uninstall.
  ${EndIf}
